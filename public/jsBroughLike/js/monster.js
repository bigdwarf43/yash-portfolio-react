...
class Player extends Monster{
    ...
    castSpell(index, manualSpellName){
        let spellName;
        if(manualSpellName==""|| manualSpellName==null){
         spellName = this.spells[index];

        }else{
            spellName = manualSpellName;
        }
        if(spellName){
    ...
    findNextTile(){
        ...
        // var path = this.finder.findPath(this.tile.x, this.tile.y, this.exitTile.x, this.exitTile.x, this.grid);
...
class Summoner extends Monster{
    constructor(tile){
        super(tile, 17, 1);
    }
    ...
    doStuff(){
        let neighbors = this.tile.getAdjacentNeighbors().filter(t => !t.passable && inBounds(t.x,t.y));
        if(neighbors.length){
            neighbors[0].summon(shuffle([Bird, Snake, Tank, Eater, Jester])[0]);
